...
def remove_dub_record():
    ...
    cur = conn.cursor()
    cur.execute("""SELECT iri,COUNT(*) AS "Number of iri"  FROM Test_data_publication  GROUP BY iri;""")
    rows = cur.fetchall()
    ...
    for item in list(set(more_thanone)):
        print(item)
        query = """SELECT Corpus,iri,auxnr,title, reference FROM Test_data_publication  WHERE iri LIKE %s;"""
        ...
        if len(df1[df1[4]!="NoData"])!=0:
            ...
            query = """DELETE FROM Test_data_publication WHERE auxnr <> %s AND iri LIKE %s;"""
