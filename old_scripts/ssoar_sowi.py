...
def sowi_ssoar_match():
    #match_info = pd.read_csv("source_ssoar.csv", sep=";")
    match_info = pd.read_csv("2018-03-06-reference_ssorid_matchedid.csv", sep=";")
    match_info=match_info[["ref_id","ssoar_id","ref_text","match_id"]]
    sowiport_info = pd.read_json("select.json")
    embargo = list(pd.read_csv("embargoHandles19feb2018.txt")["header"].apply(np.int64))
