...
def loaddata_from_csv_to_table(Corpus_id_name, flag_source_dir=0, file_fulldirectory=""):
    ...
    for item in br_json1['@graph']:
        ...
        DATA_QUERY=(str(temp['Corpus']),str(temp['iri']),str(temp['br_value']),str(temp['a']),str(temp['contributor']),str(temp['date']),str(temp['identifier']),str(temp['label']),str(temp['title']),str(temp['citation']),str(temp['reference']),uniquer_token, str(temp['iri'])[4:8])
    
        if count%1000==0:
            print(count)
    
        cur = conn.cursor()
        query = """INSERT INTO Test_data_publication(Corpus,iri, br_value,a ,contributor ,date ,identifier ,label ,title ,citation ,reference, inserttoken, ex_mat_cor) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s,%s,%s)"""
